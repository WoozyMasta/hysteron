...
func TestUpdateCluster(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for _, db := range tt.cd.DBs {
			s.dbConvergenceInfos[db.UID] = &DBConvergenceInfo{
				Generation: 0,
				Timer:      time.Now().Add(-1000 * time.Hour),
			}
		}
...
func TestSetDBSpecFromClusterSpecLogicalSlotFailoverPropagation(t *testing.T) {
	s := &Sentinel{}
	cs := &cluster.ClusterSpec{
		InitMode:                  cluster.ClusterInitModeP(cluster.ClusterInitModeNew),
		Role:                      cluster.ClusterRoleP(cluster.ClusterRoleMaster),
		...
		AdditionalWalSenders:      cluster.Uint16P(1),
		EnableLogicalSlotFailover: true,
		ManagedLogicalReplicationSlots: []cluster.ManagedLogicalReplicationSlot{
			{Name: "hysteron_logic01", Database: "postgres", Plugin: "pgoutput"},
		},
	}
	cd := &cluster.ClusterData{
		Cluster: &cluster.Cluster{
	...
	if len(cd.DBs["db1"].Spec.ManagedLogicalReplicationSlots) != 1 {
		t.Fatalf("expected managed logical slots on primary when gate is enabled")
	}
	if len(cd.DBs["db2"].Spec.ManagedLogicalReplicationSlots) != 1 {
		t.Fatalf("expected managed logical slots on standby when gate is enabled")
	}
}
...
func TestActiveProxiesInfos(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			proxyInfoHistories:         ProxyInfoHistories{"proxy1": &ProxyInfoHistory{ProxyInfo: &proxyInfo1, Timer: time.Now().Add(-45 * time.Second)}, "proxy2": &ProxyInfoHistory{ProxyInfo: &proxyInfo2, Timer: time.Now().Add(-15 * time.Second)}},
			proxiesInfos:               cluster.ProxiesInfo{"proxy1": &proxyInfo1, "proxy2": &proxyInfo2},
...
func TestRunLeadershipSanitySweepResetsTransientState(t *testing.T) {
	...
	s := &Sentinel{
		keeperErrorTimers: map[string]time.Time{"keeper1": time.Now()},
		dbErrorTimers:     map[string]time.Time{"db1": time.Now()},
		dbNotIncreasingXLogPos: map[string]int64{
			"db1": 3,
		},
		dbConvergenceInfos: map[string]*DBConvergenceInfo{
			"old": {Generation: 1, Timer: time.Now()},
		},
