...
### Initialize a new hysteron cluster with a new postgres db cluster

You can initialize new hysteron cluster with a new postgres db cluster using

```shell
hysteron cluster initialize
```

This is the same as passing a cluster specification with `initMode` set to `new`:

```shell
hysteron cluster initialize '{ "initMode": "new" }'
```
...
### Initialize a new hysteron cluster using an existing keeper
...
### First time initialization without `hysteron cluster initialize`

You can also provide `--initial-cluster-spec` to `hysteron sentinel` runtime
options, but this works only when clusterdata in the store is empty.
