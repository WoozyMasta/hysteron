...
func makeKeeperStatus(cd *cluster.ClusterData) []KeeperStatus {
	...
	keepers := make([]KeeperStatus, 0, len(cd.Keepers))
	for _, keeperUID := range cd.Keepers.SortedKeys() {
...
func keeperTreeLines(masterDBUID string, cd *cluster.ClusterData) []string {
	if masterDBUID == "" || cd == nil || cd.DBs == nil {
		return nil
	}
	var lines []string
	appendTreeLine(&lines, masterDBUID, cd, 0, "", true)
	return lines
}
...
func appendTreeLine(
	...
	*lines = append(*lines, line)
	...
	for index, followerUID := range followers {
		emptySpace := ""
		if level > 0 {
			emptySpace = "  "
		}
		lineSpace := "│ "
		isTail := index == len(followers)-1
		if tail {
			appendTreeLine(
				lines,
				followerUID,
