...
If you're going to run hysteron on an IaaS or any kind of two nodes (baremetal, VM) this document provides some example configurations. These examples provide a two nodes setup that can be extended to a number greater than two nodes.

The related store should live outside the instances hosting the hysteron cluster for multiple reasons:
...
* You can run multiple hysteron clusters using the same store

## Two nodes with load balancer

![Two nodes with a load balancer](twonodes_lb.svg)

Every nodes will run one keeper, one sentinel and one proxy.

The load balancer will balance between the hysteron proxies.

Clients will connect to the load balancer VIP.

## Two nodes with keepalived

![Two nodes with keepalived](twonodes_keepalived.svg)
