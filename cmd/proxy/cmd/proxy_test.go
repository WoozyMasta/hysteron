...
func TestValidateProxyListeners(t *testing.T) {
	tests := []struct {
		...
		{
			name: "conflicting read-only primary policy",
			cfg: config{
				ReadOnly: readOnlyOptions{
					Port:           "5433",
					NoFallback:     true,
...
func TestReadOnlyDestinationsReplicaPriority(t *testing.T) {
	tests := []struct {
		name     string
		priority readOnlyReplicaPriority
		want     []string
	}{
		{
			name:     "sync first",
			priority: readOnlyReplicaPrioritySync,
			want:     []string{"127.0.0.11:5432"},
		},
		{
			name:     "async first",
			priority: readOnlyReplicaPriorityAsync,
			want:     []string{"127.0.0.12:5432"},
		},
		{
			name:     "any",
			priority: readOnlyReplicaPriorityAny,
...
func TestReadOnlyDestinationsLagAndFallback(t *testing.T) {
	...
	c := testReadOnlyClusterChecker(readOnlyOptions{
		MaxLag:          0,
		ReplicaPriority: readOnlyReplicaPrioritySync,
	})
	got := tcpAddrStrings(c.readOnlyDestinations(cd, cd.DBs["db-primary"]))
...
func TestReadOnlyOnlyCheckDoesNotRegisterWritableProxy(t *testing.T) {
	...
	c := &ClusterChecker{
		...
		readOnlyOptions:    readOnlyOptions{ReplicaPriority: readOnlyReplicaPrioritySync},
		stopListening:      true,
		e:                  store,
